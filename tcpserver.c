...
int main(int argc, char **argv) {
    ...
    printf("[*] Server has started with ip: %s \n",
    ...
    if((newSock = accept(sock, (struct sockaddr*)&clientAddr, &clLen)) < 0) {
    ...
    while(1) {
        ...
        bytesReceived = recvfrom(newSock, (char*)buffer, MAXBYTE,0,
        ...
        if(strlen(buffer) > 0) {
            ...
            if(strncmp(buffer, "start",5) == 0) {
                if(start != 1) {
                    ...
                    sendto(newSock, message, strlen(message), 0, 
